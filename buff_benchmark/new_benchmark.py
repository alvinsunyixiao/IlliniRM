...
black_background = np.zeros((_resolution_height, _resolution_width, 3), np.uint8)
all_white_background = np.ones((_final_resolution_height, _resolution_width, 3), np.uint8)
...
def random_image_selector(desired_number, desire_width, desire_height):
    image_file_path = all_image_dir + str(desired_number) + '.jpg'
    ...
    while True:
        ...
        for cur_row in range(1, 4):
            for cur_col in range(1, 4):
                jackpot_number = random.choice(available_number) #select a number
