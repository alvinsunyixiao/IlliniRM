...
def digit_recognition(padded_num):
    #print padded_num.mean()
    if padded_num.mean() >= one_threshold:
        return 1
    #thresh_value, padded_num = cv2.threshold(padded_num, 20, 255, cv2.THRESH_BINARY)
    ...
    segments = [
        ((u_x + bias, u_y + tube_height), (u_x + tube_width, u_y + height // 2 - tube_height)),	# top-left
        ((u_x + bias, u_y + height // 2 + bias), (u_x + tube_width, u_y + height - tube_width - 1)),	# bottom-left
        ((u_x + width - tube_width - bias, u_y + tube_height), (u_x + width, u_y + height // 2 - tube_height)),	# top-right
        ((u_x + width - tube_width - bias, u_y + height // 2 + bias), (u_x + width, u_y + height - tube_width - 1)),	# bottom-right
        ((u_x + tube_width, u_y + bias), (u_x + width - tube_width - bias, u_y + tube_height)),	# top
    ...
    for (i, ((xA, yA), (xB, yB))) in enumerate(segments):
        ...
        try:
            ...
            if total / float(area) >= 0.69: on[i]= 1
